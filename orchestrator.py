...
수식이나 계산 과정을 쓸 때 LaTeX 표기를 쓰지 않는다. 채점은 문자열을 그대로
비교하므로 "120\%"처럼 백슬래시가 붙으면 "120%"와 다른 문자열이 되어
틀린 것으로 채점된다. 곱셈은 x, 나눗셈은 ÷ 또는 /, 퍼센트는 % 로 그대로 쓴다.
  (X) \(900만원 \div (11-1) \times 120\%\) = 1,200만원
  (O) 900만원 ÷ (11-1) x 120% = 1,200만원
